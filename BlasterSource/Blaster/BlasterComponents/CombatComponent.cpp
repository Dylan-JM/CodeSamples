...
void UCombatComponent::Fire()
{
	if (CanFire())
	{
		...
		if (EquippedWeapon)
		{
			CrosshairShootingFactor = CrosshairShootingFactorBase;
...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	if (WeaponToEquip->GetWeaponType() == EWeaponType::EWT_Flag)
	{
		...
		AttachFlagToBackpack(WeaponToEquip);
	...
	else
	{
		if (EquippedWeapon != nullptr && SecondaryWeapon == nullptr)
		{
			...
			WeaponToEquip->ClientUpdateHUDAmmo(WeaponToEquip->GetAmmo());
		}
		else
		{
...
void UCombatComponent::AttachActorToLeftHand(AActor* ActorToAttach)
{
	...
	FName SocketName;
	if (bUsePistolSocket) SocketName = FName("PistolSocket");
	else if (bUseSMGSocket) SocketName = FName("SMGSocket");
	else SocketName = FName("LeftHandSocket");
...
void UCombatComponent::AttachFlagToBackpack(AWeapon* Flag)
{
...
void UCombatComponent::ThrowGrenade()
{
	...
	if (CombatState == ECombatState::ECS_ThrowingGrenade || EquippedWeapon == nullptr) return;
...
void UCombatComponent::JumpToShotgunEnd()
{
	...
	if (AnimInstance && Character->GetReloadMontage() && AnimInstance->Montage_IsPlaying(Character->GetReloadMontage()))
	{
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			CrosshairShootingFactor = FMath::FInterpTo(CrosshairShootingFactor, 0.f, DeltaTime, 25.f);
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	...
	{
		EquippedWeapon->SphereRadius = bIsAiming ? EquippedWeapon->AimSphereRadius : EquippedWeapon->HipSphereRadius;
	}
...
void UCombatComponent::OnRep_HoldingTheFlag()
{
	...
	{
		// Lecture used to crouch on all but I don't want flag bearer to crouch, disable fire etc, maybe try other things though
	}
}
