...
{
	/*use a bool to call this once, otherwise when we set blasterownercontroller to null
	when we drop the weapon, this tick function will keep getting called */
...
void AWeapon::SetHUDAmmo()
{
	...
	if (BlasterOwnerCharacter)
	{
		...
		{
			//FString AmmoDisplay = FString::FromInt(Ammo);
			//if (HasAuthority()) GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Yellow, AmmoDisplay);
			//if(!HasAuthority()) GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Blue, AmmoDisplay);
...
void AWeapon::ClientUpdateHUDAmmo_Implementation(int32 ServerAmmo)
{
	if (HasAuthority()) return;
	Ammo = ServerAmmo;
	SetHUDAmmo();
