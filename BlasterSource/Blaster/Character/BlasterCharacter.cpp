...
ABlasterCharacter::ABlasterCharacter()
{
	...
	Combat->StartingARAmmo = 60;

	Buff = CreateDefaultSubobject<UBuffComponent>(TEXT("BuffComponent"));
	Buff->SetIsReplicated(true);

	LagCompensation = CreateDefaultSubobject<ULagCompensationComponent>(TEXT("LagCompensatoin"));
...
void ABlasterCharacter::BeginPlay()
{
	...
	UpdateHUDHealth();
	UpdateHUDShield();
	// Update HUD ammo after really short period of timer due to PlayerController being null when trying to call it.
	GetWorldTimerManager().SetTimer(RespawnTimer, this , &ABlasterCharacter::UpdateHUDAmmo, 0.1f);
	//UpdateHUDAmmo();
	if (HasAuthority())
	{
...
void ABlasterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (TObjectPtr<UEnhancedInputComponent> EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(StopFireAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::FireButtonReleased);
...
void ABlasterCharacter::PlayReloadMontage()
{
	...
	if (AnimInstance && ReloadMontage)
	{
		...
		AnimInstance->Montage_JumpToSection(SectionName);
		AnimInstance->OnMontageEnded.AddDynamic(this, &ABlasterCharacter::ReloadMontageEndedHandler);
	}
}
                                      ...
                                      AController* InstigatorController, AActor* DamageCauser)
{
	...
	if (Health == 0.f)
	{
		if (BlasterGameMode)
		{
			...
			ABlasterPlayerController* AttackerController = Cast<ABlasterPlayerController>(InstigatorController);BlasterGameMode->PlayerEliminated(this, BlasterPlayerController, AttackerController);
		}
	}
}
...
void ABlasterCharacter::FireButtonReleased()
{
	...
	{
		Combat->FireButtonPressed(false);
		//GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Yellow, TEXT("Released"));
	}
}
...
{
	BlasterPlayerController = BlasterPlayerController == nullptr ? Cast<ABlasterPlayerController>(Controller) : BlasterPlayerController;
	//if (BlasterPlayerController) GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Yellow, "PlayerController good");
	//if (Combat) GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Yellow, "Combat good");
	//if (Combat->EquippedWeapon) GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Yellow, "Combat Equipped weapon good");
	if (BlasterPlayerController && Combat && Combat->EquippedWeapon)
	{
		//FString DisplayAmmo = FString::FromInt(Combat->EquippedWeapon->GetAmmo());
		//if (HasAuthority()) GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Yellow, DisplayAmmo);
...
{
	{
	   ...
	   if(bInterrupted)
	   {
	      ...
	      AnimInstance->OnMontageEnded.RemoveDynamic(this, &ABlasterCharacter::ReloadMontageEndedHandler);
